...
- [ ] **Task 1: Review Existing Dashboard Implementation** (AC: 1-5)
  - [ ] Review current `apps/web/app/dashboard/page.tsx` implementation
  - [ ] Identify what's already implemented vs. missing from ACs
  - [ ] Review Clerk middleware configuration in `middleware.ts`
  - [ ] Check if dashboard layout file exists
  - [ ] Document current state vs. desired state

- [ ] **Task 2: Create Dashboard Layout Component** (AC: 1, 5)
  - [ ] Create `apps/web/app/dashboard/layout.tsx` with shared header/sidebar structure
  - [ ] Implement dashboard header with:
    - [ ] Application logo/title
    - [ ] User avatar/name display using Clerk's `useUser()` hook
    - [ ] Logout button in header calling `signOut()` from `useClerk()` hook
  - [ ] Use shadcn/ui Button component for logout action
  - [ ] Apply responsive design for mobile/tablet/desktop breakpoints
  - [ ] Use Tailwind CSS for layout styling (grid or flexbox)
  - [ ] Ensure layout wraps all dashboard pages (children prop)

- [ ] **Task 3: Update Dashboard Page with Initiative List Placeholder** (AC: 3, 4)
  - [ ] Update `apps/web/app/dashboard/page.tsx` to remove duplicate user display (now in layout)
  - [ ] Add welcome message with user's first name prominently displayed
  - [ ] Create placeholder section for Initiative list with:
    - [ ] Card component from shadcn/ui
    - [ ] "Your Initiatives" heading
    - [ ] Empty state message: "No initiatives yet. Create your first initiative in Epic 2!"
    - [ ] Placeholder "Create Initiative" button (disabled/coming soon state)
  - [ ] Use semantic HTML and proper ARIA labels for accessibility

- [ ] **Task 4: Verify Route Protection and Authentication Flow** (AC: 2)
  - [ ] Test that unauthenticated users are redirected to `/sign-in` when accessing `/dashboard`
  - [ ] Verify Clerk middleware is protecting the dashboard route
  - [ ] Test authentication flow: sign up → redirect to dashboard
  - [ ] Test logout flow: sign out → redirect to homepage
  - [ ] Ensure loading states are properly displayed during auth checks

- [ ] **Task 5: Write Unit Tests for Dashboard Layout** (AC: 1, 3, 5)
  - [ ] Create test file: `apps/web/app/dashboard/__tests__/layout.test.tsx`
  - [ ] Test layout renders with user information (mock `useUser()` hook)
  - [ ] Test logout button calls `signOut()` when clicked
  - [ ] Test header displays user's first name correctly
  - [ ] Test layout renders children components
  - [ ] Mock Clerk hooks (`useUser`, `useClerk`) in tests

- [ ] **Task 6: Write Unit Tests for Dashboard Page** (AC: 3, 4)
  - [ ] Update test file: `apps/web/app/dashboard/__tests__/page.test.tsx`
  - [ ] Test dashboard page renders welcome message with user's first name
  - [ ] Test initiative list placeholder is displayed
  - [ ] Test "Create Initiative" button is present (disabled state)
  - [ ] Test loading state when user is not yet loaded
  - [ ] Mock Clerk `useUser()` hook with test user data

- [ ] **Task 7: Write Integration Tests for Authentication Flow** (AC: 2)
  - [ ] Create E2E test file: `apps/web/e2e/dashboard-auth.spec.ts`
  - [ ] Test unauthenticated user is redirected to `/sign-in` when accessing `/dashboard`
  - [ ] Test authenticated user can access dashboard
  - [ ] Test logout button redirects to homepage
  - [ ] Use Playwright for E2E testing
  - [ ] Set up test fixtures for authenticated/unauthenticated states

- [ ] **Task 8: Style Dashboard with Consistent Design System** (AC: 1, 3, 4, 5)
  - [ ] Apply neutral theme colors from `tailwind.config.ts`
  - [ ] Use shadcn/ui components (Button, Card, Separator) for consistent styling
  - [ ] Ensure proper spacing and typography using Tailwind utilities
  - [ ] Test dark mode compatibility (if dark mode toggle exists)
  - [ ] Verify responsive design on mobile (320px), tablet (768px), desktop (1440px)

- [ ] **Task 9: Verify Accessibility Standards** (AC: 1-5)
  - [ ] Test keyboard navigation: Tab through header elements, Enter/Space to trigger logout
  - [ ] Verify logout button has proper ARIA label
  - [ ] Test with screen reader (VoiceOver on macOS or NVDA on Windows)
  - [ ] Ensure focus indicators are visible on interactive elements
  - [ ] Verify heading hierarchy (h1 for main dashboard title)

- [ ] **Task 10: Run Build and Linting** (AC: 1-5)
  ...
  - [ ] Ensure no console errors in browser when viewing dashboard
