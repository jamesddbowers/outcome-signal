...
describe('MiddlePanel', () => {
  ...
  it('renders with initiativeId prop', () => {
    ...
    expect(screen.getAllByText(/Document Preview/i)[0]).toBeInTheDocument();
  ...
  it('displays placeholder content', () => {
    ...
    expect(
      screen.getAllByText(/Placeholder for document preview with TipTap editor \(Story 2.3\)/i)[0]
    ).toBeInTheDocument();
  });
  ...
  it('renders with proper heading structure', () => {
    ...
    const heading = screen.getAllByText(/Document Preview/i)[0];
    expect(heading).toBeInTheDocument();
    expect(heading.tagName).toBe('H2');
  });

  it('uses Card component for layout', () => {
    const { container } = render(<MiddlePanel initiativeId={testInitiativeId} />);
